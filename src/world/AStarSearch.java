...
public class AStarSearch {
    ...
    public static Path search(Cell start, Cell target, int maxDistance) {
        ...
        if (start == null || target == null) {
            RescueFramework.log("AStarSearch failed: start or target is null");
            return null;
        }

        // Add distance validation
        if (maxDistance != -1 && maxDistance < 0) {
            RescueFramework.log("AStarSearch failed: invalid maxDistance");
            return null;
        }
        ...
        if (verbose)
            RescueFramework.log("AStarSearch between " + start.getX() + "x" + start.getY() + " and " + target.getX()
                    + "x" + target.getY() + " max distance " + maxDistance);
        ...
        while (openList.size() > 0) {
            ...
            if (verbose)
                RescueFramework.log("  Processing node " + selectedCell.getCell().getX() + " x "
                        + selectedCell.getCell().getY() + " ("
                        + (Math.round(100 * (selectedCell.getSumG() + selectedCell.getH())) / 100d) + ")");

            // Check max distance constraint
            if (maxDistance > 0 && selectedCell.getSumG() > maxDistance) {
                if (verbose)
                    RescueFramework.log("    Max distance reached (" + maxDistance + "), giving up!");
                return null;
            }
            ...
            if (selectedCell.getCell().equals(target)) {
                if (verbose)
                    RescueFramework.log(
                            "    Target reached: " + target.getX() + " x " + target.getY() + ". Building path back:");
                Path result = new Path();
                while (selectedCell != null) {
                    if (verbose)
                        System.out
                                .print(selectedCell.getCell().getX() + " x " + selectedCell.getCell().getY() + " --> ");
                ...
                if (possibleNeighbour != null) {
                    if (possibleNeighbour.isDiscovered() || possibleNeighbour.equals(target)) {
                        ...
                        if (!possibleNeighbour.hasObstacle() || possibleNeighbour.equals(target)) {
                            if (!possibleNeighbour.hasFire() || possibleNeighbour.equals(target)) {
                                boolean skip = false;
                                ...
                                if (skip) {
                                    if (verbose)
                                        RescueFramework.log("    Already on open list.");
                                    continue;
                                }
                                ...
                                if (skip) {
                                    if (verbose)
                                        RescueFramework.log("    Already on closed list.");
                                    continue;
                                }

                                // Add possible neighbour to open list
                                if (verbose)
                                    RescueFramework.log("    Adding to open list!");
                                openList.add(new AStarCell(possibleNeighbour, selectedCell.getSumG() + 1,
                                        heuristics(possibleNeighbour, target), selectedCell));
                            } else {
                                if (verbose)
                                    RescueFramework.log("    Occupied by fire.");
                            }
                        } else {
                            if (verbose)
                                RescueFramework.log("    Occupied by obstacle.");
                        }
                    } else {
                        if (verbose)
                            RescueFramework.log("    Not discovered yet.");
                    }
                } else {
                    if (verbose)
                        RescueFramework.log("    No neighbour in dir " + dir + ".");
                }
            }
    ...
    public static String listToString(ArrayList<AStarCell> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            result = result + cell.getCell().getX() + "x" + cell.getCell().getY() + "(" + g + "+"
                    + (Math.round(100 * (h)) / 100d) + "=" + (Math.round(100 * (g + h)) / 100d) + "), ";
        }
        return result;
    }
}
