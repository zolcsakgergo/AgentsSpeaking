...
public class PaintPanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int x = 0; x < map.getWidth(); x++) {
            for (int y = 0; y < map.getHeight(); y++) {
                ...
                if (!cellImage.isEmpty()) {
                    g2.drawImage(map.getCachedImage(cellImage), x * cellSize + 1, y * cellSize + 1, cellSize - 1,
                            cellSize - 1, null);
                }

                // Display injured on the cell
                drawFire(g2, cell, cellSize, cell.getFire());

                // Display wall code on the firs row
                /*
                 * if (y == 0) {
                 * if (cell.getWallCode()>0) g2.drawString(String.valueOf(cell.getWallCode()),
                 * x*cellSize+15, y*cellSize+23);
                 * }
                 */
        ...
        // Y axis
        for (gridX = 0; gridX <= map.getWidth(); gridX++) {
            g2.drawLine(gridX * cellSize, 0, gridX * cellSize, map.getHeight() * cellSize);
        }

        // X axis
        for (gridY = 0; gridY <= map.getHeight(); gridY++) {
            g2.drawLine(0, gridY * cellSize, map.getWidth() * cellSize, gridY * cellSize);
        }

        // Draw walls where needed after all
        for (gridX = 0; gridX < map.getWidth(); gridX++) {
            for (gridY = 0; gridY < map.getHeight(); gridY++) {
                Cell cell = map.getCell(gridX, gridY);
                if (RescueFramework.mainFrame.isFogEnabled()) {
                    if (cell.isDiscovered()) {
                        drawCellWalls(g2, cell, cellSize);
                    }
     ...
     * @param fire     The fire to display
     */
    public void drawFire(Graphics2D g, Cell cell, int cellSize, Fire fire) {
        if (fire == null)
            return;
        ...
        // Choose fire image based on intensity
        String fireImage = "fire";

        // Draw the image
        g.drawImage(RescueFramework.map.getCachedImage(fireImage),
                x * cellSize + 6,
                y * cellSize + 2,
                cellSize - 6,
                cellSize - 12,
        ...
        float intensityPercent = fire.getIntensity() / 1000f;
        int barWidth = (int) ((cellSize - 9) * intensityPercent);
        g.fillRect(x * cellSize + 4, (y + 1) * cellSize - 9, barWidth, 5);
    }
